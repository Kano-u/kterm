...
- [x] 收到 `{"t":"exit"}`：status='ended'，清理终端条目；若当前在该终端视图 → 自动切回 `view='files'` + toast「终端会话已结束」
- [x] `closeTab(id)`：先 ws.close() 杀终端再删标签
- [x] 关闭文件标签 → 服务端 PTY 立即退出（空闲终端）
- [x] `switchTab` 在终端视图下：切到新标签的终端层，无会话则自动建连
- [x] 刷新后：terminals 表清空，重新进入终端视图自动新建会话
- [x] `beforeunload`：任一终端 busy 时拦截刷新/关闭浏览器页
- [x] 多浏览器窗口同一 tabId 二次连接：拒绝并提示「该标签的终端已被其他窗口占用」
- [x] 手测：exit 后切回与重建、关标签杀进程（任务管理器确认无残留）、刷新拦截生效
