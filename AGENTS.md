...
- **CSS 层叠**：`style.css` 里的自定义类必须放进 `@layer components`（或更早的层）。
  写成顶层无层级规则时，编译产物会把它排在 `@layer utilities` **之后**；特异性同为 (0,1,0)
  时靠后者胜出，遂静默盖掉元素上的 Tailwind 工具类。曾因此出现：按钮写了
  `state-layer absolute right-3 top-2`，`.state-layer` 的 `position:relative` 把
  `absolute` 盖掉，按钮停在普通流左侧（源码完全看不出来）。`cascade.test.mjs` 静态分析
  编译产物锁住该契约，`cascade-negative.mjs` 用故意写坏的 CSS 自检「测试本身有效」。
