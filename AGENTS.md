...
└── frontend/               # Vue 3 + Tailwind + Vite 源码
    ...
    ├── test/               # 纯 node 回归测试（settingsnav/editor/touchscroll/termgutter）
    └── src/
...
端点：`GET /api/list`、`GET /api/search`、`POST /api/mkdir`、`/api/create`、`/api/rename`、`/api/copy`、`/api/move`、`/api/delete`（`mode:"trash"|"permanent"`）、`GET /api/trash`、`POST /api/trash/restore`、`/api/trash/purge`、`GET /api/read`、`POST /api/write`、`GET /api/term/ws`（终端 WebSocket，见下）。

### 编辑器端点（/api/read、/api/write）

服务端无会话状态，只做纯文件读写：

- `GET /api/read?path=<相对路径>` → `{"content":"...","mtime":<unix 毫秒>,"size":N}`。
